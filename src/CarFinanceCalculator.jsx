...
const CarFinanceCalculator = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await fetch(
        "https://6701eeb6b52042b542d8d315.mockapi.io/carTaxes"
      );
  ...
  //input currency formatter realtime changes input as a currency
  ...
  //output currency formatter because this way is not real time
  ...
  const data = [
    {
      name: "Loan amount without the vehicle tax",
      value: parseFloat(price) - parseFloat(downPayment),
    },
    { name: "Down Payment", value: parseFloat(downPayment) },
    { name: "Interest", value: parseFloat(totalInterestPaid) },
    { name: "Tax", value: parseFloat(vehicleTax) },
  ];

  const data1 = [
    {
      name: "Total payments for the vehicle",
